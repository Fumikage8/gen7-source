...
namespace gfl2 {
  namespace ui {
    ...
    TouchPanel::TouchPanel( 
        gfl2::heap::HeapBase* heap, DeviceImplementer* implementer ) :
     ...
     */
    //-------------------------------------------------------------------------
    void TouchPanel::GetXY( gfl2::math::VEC3* vec ) const
    {
      vec->x = this->GetX();
      vec->y = this->GetY();
    } 
     ...
     */
    //-------------------------------------------------------------------------
    void TouchPanel::GetPrevXY( gfl2::math::VEC3* vec ) const
    {
      vec->x = this->GetPrevX();
      vec->y = this->GetPrevY();
    }
     ...
     */
    //-------------------------------------------------------------------------
    void TouchPanel::GetMoveXY( gfl2::math::VEC3* vec ) const
    {
      vec->x = this->GetMoveX();
      vec->y = this->GetMoveY();
    }
    ...
    bool TouchPanel::CheckDClickRange( u16 x, u16 y ) const
    {
      ...
      {
        return ( 
            ( static_cast<u32>( gfl2::math::Abs(diff_x) ) <= m_dclick_cond.radius ) &&
            ( static_cast<u32>( gfl2::math::Abs(diff_y) ) <= m_dclick_cond.radius ) );
      }
      else
      {
        ...
        void* buffer, const void* detective_data, const void* prev_detective_data ) const
    {
      ...
      else
      {
        ...
        buffer_data->repeat  |= m_pRepeatTimer->IsRepeat() ? now_data->touch : false;
      }
    }
