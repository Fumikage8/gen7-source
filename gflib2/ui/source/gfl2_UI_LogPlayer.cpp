...
namespace gfl2 {
  namespace ui {
    ...
    {
      GFL_PRINT("LogPlayer::SetPlayPos %d \n", pos );
      m_playDataIdx = pos;
    }
    ...
    {
      GFL_PRINT("LogPlayer::SetPlayEndPos %d \n", endPos );
    ...
    bool LogPlayer::IsPlayEnd( void ) const
    {
      ...
      if( ( 0 <= m_maxPlayDataIdx ) && ( static_cast<u32>( m_maxPlayDataIdx ) < m_playDataIdx ) ) {
        GFL_PRINT("LogPlayer::IsPlayEnd a %d %d \n", m_maxPlayDataIdx, m_playDataIdx );
        return true;
      }

      const u32 log_data_num = m_pRecorder->GetLogDataNum();
      if( log_data_num <= m_playDataIdx ) {
        GFL_PRINT("LogPlayer::IsPlayEnd b %d %d \n", log_data_num, m_playDataIdx );
        return true;
      }
