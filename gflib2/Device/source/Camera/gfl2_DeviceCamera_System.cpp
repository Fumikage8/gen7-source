...
/////////////////////////////////////////////////////////////////////////////////////////
/**
 * @file   gfl2_DeviceCamera_System.cpp
 * @brief  カメラシステム
...
GFL_NAMESPACE_BEGIN(camera)
        ...
        m_errorBitFlag(ERROR_BIT_NONE)
      {
        ...
        if( m_y2rThread )
        {
          m_y2rThread->Start( param.y2rThreadPriority );
        }
      ...
      BufferManager* System::CreateBuffer( const CameraParam& param )
      {
        ...
        gfl2::device::camera::BufferManager::SetupParam buf_param;
        buf_param.pHeap         = m_pDeviceHeap;
        ...
        {
          buf_param.yuvBufferSize = param.trimmingWidth * param.trimmingHeight * 4;  // @attention CTRでない場合要注意！ 注意喚起のためこのtodoは残しておきます。  // CTR CTRプログラミングマニュアル システム編 2014-01-21 Ver.7.0  カメラのキャプチャ画像は YUV 一括のフォーマットでのみ取得することができますので、ほとんどの場合、キャプチャ画像の変換時には INPUT_YUV422_BATCH を指定することになります。
          buf_param.rgbBufferSize = param.outputWidth * param.outputHeight * OUTPUT_BIT_PER_PIXEL[param.outputFormat]/8;
        }

        return GFL_NEW(m_pHeap) gfl2::device::camera::BufferManager( buf_param );
      }
