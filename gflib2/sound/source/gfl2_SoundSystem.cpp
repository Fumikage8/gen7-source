...
#define GFL_SOUNDSYSTEM_PLAYER_HEAP_SIZE_CHECK 0
 ...
 #define CRITICALSECTION_ENTER() m_criticalSectionForSoundDataManagerLoad.Enter()
 #define CRITICALSECTION_LEAVE() m_criticalSectionForSoundDataManagerLoad.Leave()
#elif defined(GF_PLATFORM_WIN32)
 //GFNMCat[1377] LoadData()とnw::sndの内部スレッドの競合を回避するための非公開APIを任天堂様に教えていただき適用(ml-poketech:2164)
 //CTRではスレッドプライオリティで割り込みを制限できるが、Win32だとスレッドがCPUを占有できないため明示的に回避する
 #define CRITICALSECTION_ENTER() nw::snd::internal::TaskThread::GetInstance().Lock() //非公開API
 #define CRITICALSECTION_LEAVE() nw::snd::internal::TaskThread::GetInstance().Unlock()
 #include "windows.h"
#endif
...
*/
//-----------------------------------------------------------------------------
SINGLETON_INSTANCE_DEFINITION(gfl2::sound::SoundSystem)
template class gfl2::base::SingletonAccessor<gfl2::sound::SoundSystem>;
...
namespace gfl2 {
  namespace sound {
...
#elif defined(GF_PLATFORM_WIN32)
      // サウンドアーカイブファイルを開く
      bool open_succeeded = m_fsSoundArchive.Open( initParam.bfsarFilePath );
      GFL_ASSERT_STOP( open_succeeded );  //@check

      // サウンドアーカイブのヘッダを読み込む
      size_t header_size = m_fsSoundArchive.GetHeaderSize();
      m_pMemoryForSoundArchiveHeader = GflHeapAllocMemoryAlign( m_pDevHeap, header_size, nw::snd::FsSoundArchive::BUFFER_ALIGN_SIZE );
      bool load_succeeded = m_fsSoundArchive.LoadHeader( m_pMemoryForSoundArchiveHeader, header_size );
      GFL_ASSERT_STOP( load_succeeded ); //@check
#endif
    ...
    void SoundSystem::InitSoundDataManager( void )
    {
      ...
      /*bool suceeded = */(void)m_soundDataManager.Initialize( //戻り値使ってないのでコメントアウト
          m_pSoundArchive, 
...
#elif defined(GF_PLATFORM_WIN32)
      m_pMemoryForStreamBuffer = GflHeapAllocMemoryAlign( m_pDevHeap, stream_buffer_size, 8 ); // アプリケーションヒープはNG
      m_pMemoryForCacheBuffer = GflHeapAllocMemoryAlign( m_pDevHeap, cache_buffer_size, nw::snd::FsSoundArchive::BUFFER_ALIGN_SIZE ); // アプリケーションヒープはNG
#endif
    ...
    { 
      if( 0 < initParam.soundHeapSize ) {
        m_soundHeap = GFL_NEW( m_pAppHeap ) gfl2::sound::SoundHeap( m_pAppHeap, m_pDevHeap, initParam.soundHeapSize );
      }
    ...
    {
      // リスナー行列を設定
      gfl2::math::MTX34 listener_matrix;
      const gfl2::math::VEC3 pos( 0.0f, 0.0f, 0.0f );   // リスナーの位置
      const gfl2::math::VEC3 up( 0.0f, 1.0f, 0.0f );  // リスナーの上方ベクトル
      const gfl2::math::VEC3 dir( 0.0f, 0.0f, 1.0f ); // リスナーの向きベクトル
     ...
     */
    //-------------------------------------------------------------------------
    bool SoundSystem::LoadSoundData( u32 itemID, LoadFlags loadFlag )
    {
     ...
     */
    //-----------------------------------------------------------------------
    bool SoundSystem::LoadSoundData( SoundHeap* dest, u32 itemID, LoadFlags loadFlag )
    {
      bool result = false;
      CRITICALSECTION_ENTER();
      {
        ...
        if( nw_sound_heap->IsValid() ) {
          result = m_soundDataManager.LoadData( itemID, nw_sound_heap, static_cast<u32>(loadFlag) );
        }
     ...
     */
    //-----------------------------------------------------------------------------
    bool SoundSystem::LoadSoundDataAsync( gfl2::heap::HeapBase * thread_heap, s32 priority, u32 itemID, LoadFlags loadFlag )
    {
      GFL_ASSERT_MSG( m_soundHeap, "ERROR!! サウンドヒープが作成されていません\n" ); //@check
      if( m_soundHeap ) {
     ...
     */
    //-----------------------------------------------------------------------
    bool SoundSystem::LoadSoundDataAsync( SoundHeap* dest, gfl2::heap::HeapBase * thread_heap, s32 priority, u32 itemID, LoadFlags loadFlag )
    {
      // まだ読み込み中
    ...
    bool SoundSystem::Play( u8 handleIdx, u32 itemID, s32 fadeInFrames, const nw::snd::SoundStartable::StartInfo* startInfo )
    {
...
#if 0 //リバーブテスト用 強制エフェクトバス設定
      {
        ...
        if( sound_type == nw::snd::SoundArchive::SOUND_TYPE_SEQ ) {  // シーケンスサウンド
          handle->SetFxSend( nw::snd::AUX_BUS_A, 0.5f );
        }
      }
#endif
    ...
    f32 SoundSystem::CalcPitchByEqualTemperament( s8 level ) const
    {
      ...
      u8 abs_level = gfl2::math::Abs( level );
     ...
     */
    //-------------------------------------------------------------------------
    void SoundSystem::SetTrackPan( u8 handleIdx, u32 trackBitFlag, f32 pan )
    {
      if( m_soundHandleNum <= handleIdx ) {
        GFL_ASSERT(0); //@check
        return;
...
#elif defined(GF_PLATFORM_WIN32)
        seq_handle.SetTrackOutputPan( nw::snd::OUTPUT_DEVICE_MAIN,trackBitFlag, pan );
#endif
      }
      else if( sound_type == nw::snd::SoundArchive::SOUND_TYPE_STRM ) { // ストリームサウンド
        nw::snd::StreamSoundHandle stream_handle( handle );
...
#elif defined(GF_PLATFORM_WIN32)
        stream_handle.SetTrackOutputPan( nw::snd::OUTPUT_DEVICE_MAIN,trackBitFlag, pan );
#endif
      }
    }
...
#elif defined(GF_PLATFORM_WIN32)
        stream_handle.SetTrackOutputSurroundPan( nw::snd::OUTPUT_DEVICE_MAIN,trackBitFlag, pan );
#endif
     ...
     */
    //---------------------------------------------------------------------------
    void SoundSystem::SetTrackVariable( 
      u8 handleIdx, int trackNo, int varNo, s16 value )
    {
      if( m_soundHandleNum <= handleIdx ) {
        GFL_ASSERT(0); //@check
        return;
      ...
      if( sound_type == nw::snd::SoundArchive::SOUND_TYPE_SEQ ) { // シーケンスサウンド
        nw::snd::SequenceSoundHandle seq_handle( handle );
        seq_handle.WriteTrackVariable( trackNo, varNo, value );
      }
      else
      {
        GFL_ASSERT(0); //@check
     ...
     */
    //---------------------------------------------------------------------------
    s16 SoundSystem::GetTrackVariable( 
      u8 handleIdx, int trackNo, int varNo )
    {
      if( m_soundHandleNum <= handleIdx ) {
        GFL_ASSERT(0); //@check
        return 0;
      ...
      if( sound_type == nw::snd::SoundArchive::SOUND_TYPE_SEQ ) { // シーケンスサウンド
        nw::snd::SequenceSoundHandle seq_handle( handle );
        if( seq_handle.ReadTrackVariable( trackNo, varNo, &value ) ){
          return value;
        }
      }
      GFL_ASSERT(0); //@check
    ...
    void SoundSystem::SetTrackMute( u8 handleIdx, u32 trackBitFlag, SeqMute mute )
    {
      ...
      if( sound_type == nw::snd::SoundArchive::SOUND_TYPE_SEQ ) { // シーケンスサウンド
        nw::snd::SequenceSoundHandle seq_handle( handle );
        seq_handle.SetTrackMute( trackBitFlag, GFL_BOOL_CAST(mute) );
      }
    }
    ...
    //-----------------------------------------------------------------------
#if defined(GF_PLATFORM_CTR)
    bool SoundSystem::AppendReverbEffectCTR( nw::snd::AuxBus busID, gfl2::heap::HeapBase* reverb_buffer_heap, const nw::snd::FxReverb::Param & param )
    {
      ...
      m_fxReverbCTR.SetParam( param );
      
      // ディレイの初期化（パラメータ設定後のみ使用可能）
      size_t size = m_fxReverbCTR.GetRequiredMemSize();
      
      // エフェクトの仕様する領域を確保、割り当て
      m_pMemoryForSoundReverb = GflHeapAllocMemory( reverb_buffer_heap, size );
      m_fxReverbCTR.AssignWorkBuffer( reinterpret_cast<uptr>(m_pMemoryForSoundReverb), size );
      return nw::snd::SoundSystem::AppendEffect( busID , &m_fxReverbCTR );
    }
#elif defined(GF_PLATFORM_WIN32)
    bool SoundSystem::AppendReverbEffectWin32( nw::snd::AuxBus busID, gfl2::heap::HeapBase* reverb_buffer_heap, const nw::snd::FxReverbStd::ReverbStdParam & param )
    {
      if( m_pMemoryForSoundReverb != NULL )
      {
        GFL_ASSERT_MSG(0,"リバーブを２重にかけることはできません\n"); //@check
      ...
      m_fxReverbWin32.SetParam( param );
      
      // ディレイの初期化（パラメータ設定後のみ使用可能）
      size_t size = m_fxReverbWin32.GetRequiredMemSize();
      
      // エフェクトの仕様する領域を確保、割り当て
      m_pMemoryForSoundReverb = GflHeapAllocMemory( reverb_buffer_heap, size );
      m_fxReverbWin32.AssignWorkBuffer( m_pMemoryForSoundReverb, size );
      return nw::snd::SoundSystem::AppendEffect( busID , &m_fxReverbWin32 );
    }
#endif
     ...
     */
    //-------------------------------------------------------------------------
    void SoundSystem::SetListenerMatrix( const gfl2::math::MTX34& matrix )
    {
    ...
    {
      gfl2::math::MTX34 matrix;
      gfl2::math::VEC3 target = pos + dir;
      gfl2::math::MTX34LookAt( &matrix, &pos, &up, &target ); 
     ...
     */
    //-------------------------------------------------------------------------
    void SoundSystem::SetListenerVelocity( const gfl2::math::VEC3& velocity )
    {
    ...
    void SoundSystem::Set3DActorPan( u8 actorIdx, f32 pan )
    {
      ...
      actor->SetPan( pan );
#elif defined(GF_PLATFORM_WIN32)
      actor->SetOutputPan( nw::snd::OUTPUT_DEVICE_MAIN, pan );
#endif
    }
    ...
    f32 SoundSystem::Get3DActorPan( u8 actorIdx ) const
    {
      ...
      return actor->GetPan();
#elif defined(GF_PLATFORM_WIN32)
      return actor->GetOutputPan( nw::snd::OUTPUT_DEVICE_MAIN);
#endif
    }
     ...
     */
    //-------------------------------------------------------------------------
    void SoundSystem::Set3DActorPos( u8 actorIdx, const gfl2::math::VEC3& pos )
    {
      if( m_sound3DActorNum <= actorIdx ) {
        GFL_ASSERT(0); //@check
        return;
     ...
     */
    //-------------------------------------------------------------------------
    void SoundSystem::Get3DActorPos( u8 actorIdx, gfl2::math::VEC3* pos ) const
    {
      if( m_sound3DActorNum <= actorIdx ) {
        GFL_ASSERT(0); //@fix
        *pos = gfl2::math::VEC3::Zero();
        return;
      }
      const nw::snd::Sound3DActor* actor = &m_pSound3DActors[ actorIdx ]; 
     ...
     */
    //-------------------------------------------------------------------------
    void SoundSystem::Set3DActorVelocity( u8 actorIdx, const gfl2::math::VEC3& velocity )
    {
      if( m_sound3DActorNum <= actorIdx ) {
        GFL_ASSERT(0); //@check
        return;
     ...
     */
    //-------------------------------------------------------------------------
    nw::snd::RomSoundArchive* SoundSystem::GetRomSoundArchive( void )
    {
      return &(m_romSoundArchive);
    }
#endif //defined(GF_PLATFORM_CTR)
