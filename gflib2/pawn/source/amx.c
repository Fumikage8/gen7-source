...
//GFL_WARNING_CTR_IGNORE(193) が効かなかったのでwin32のみに
...
#if GFL_PAWN_OMIT_OFFSET
#define GETENTRYNAME(hdr,entry) \
                        ( USENAMETABLE(hdr) \
                           ? (char *)&(((AMX_FUNCSTUBNT*)(entry))->nameofs) \
...
#if BYTE_ORDER==PAWN_BIG_ENDIAN || PAWN_CELL_SIZE==16
...
#if BYTE_ORDER==PAWN_BIG_ENDIAN || PAWN_CELL_SIZE==32
...
#if (BYTE_ORDER==PAWN_BIG_ENDIAN || PAWN_CELL_SIZE==64) && (defined _I64_MAX || defined HAVE_I64)
 ...
 * @func    amx_Init
 * @brief   amx_Init関数.
  * @date   2014/10/27(月) 15:01:54
 *
 * @param[in,out] AMX*  amx   .
 * @param[in,out] void* program .
 *
 * @return    int AMXAPI  
 */
//------------------------------------------------------------------------------
...
-------------------------------------------------------------
*/
#if GFL_PAWN_USE_HASH
  memcpy(funcname,GETENTRYNAME(hdr,func),sizeof(cell));
#else
...
#define PRINT_LOG printf//NN_LOG
#else
...
#if GFL_PAWN_USE_HASH
  for (idx=0; idx<last; idx++) {
    amx_GetPublic(amx,idx,pname,NULL);
  phsh = *((unsigned int*)pname);
  if( hsh == phsh ){
    *index = idx;
    return AMX_ERR_NONE;
...
*/
#if GFL_PAWN_USE_HASH
  memcpy(varname,GETENTRYNAME(hdr,var),sizeof(cell));
#else
...
#if GFL_PAWN_USE_HASH
  for (idx=0; idx<last; idx++) {
    amx_GetPubVar(amx,idx,pname,&paddr);
  phsh = *((unsigned int*)pname);
  if( hsh == phsh ){
    *amx_addr = paddr;
    return AMX_ERR_NONE;
...
*/
#if GFL_PAWN_USE_HASH
  memcpy(tagname,GETENTRYNAME(hdr,tag),sizeof(cell));
#else
...
#if GFL_PAWN_USE_HASH
  for (idx=0; idx<last; idx++) {
    amx_GetTag(amx,idx,tagname,&mid_id);
    if(mid_id == tag_id){
    return AMX_ERR_NONE;
  }
  } /* for */
...
*/
  ...
  unsigned int hsh, phsh;
//  hsh = CalcHash(name);
  hsh = *((unsigned int*)name);
#endif
  PRINT_LOG("Check 0x%08x\n",hsh);
...
*/
  ...
  for (i=0; list[i].name!=NULL && (i<number || number==-1); i++){
    PRINT_LOG("[%d] %s ",i,list[i].name);
    phsh = CalcHash(list[i].name);
//    phsh = *((unsigned int*)list[i].name);
    if (hsh == phsh){
     PRINT_LOG("\nFound\n");
     return list[i].func;
    }
...
#else
static AMX_NATIVE findfunction( const char *name, const AMX_NATIVE_INFO *list, int number, u32 hash_tbl[] )
{
  int i;
/*
-------------------------------------------------------------
...
static void amx_UpdateStackHeap( AMX * amx );
//------------------------------------------------------------------------------
/**
 * @func    amx_Register
 * @brief   amx_Register関数.
 * @date    2014/10/27(月) 15:57:10
 *
 * @param[in,out] AMX*          amx   .
 * @param[in]   const AMX_NATIVE_INFO*  list  リスト.
 * @param[in]   int           number  .
 *
 * @return    int AMXAPI  
 */
...
int AMXAPI amx_Register( AMX *amx, const AMX_NATIVE_INFO *list, int number )
{
  ...
  //  unsigned int* hash_tbl = (unsigned int*)malloc(sizeof(unsigned int) * HASH_TBL_SIZE);
#if 0
  cell * paddr, xaddr;
  err = amx_Allot( amx, HASH_TBL_SIZE, &xaddr, &paddr );
  assert( !err );
  amx_UpdateStackHeap( amx );
  hash_tbl = ( unsigned int * )paddr;
#else
  {// 使い捨てのメモリアドレスを計算
    unsigned char * data;
    ...
    hash_tbl = ( unsigned int * )( data + ( int )amx->hea );
  }
#endif
#else
  unsigned int hash_tbl[HASH_TBL_SIZE];
#endif
  ...
  assert( 16 <= amx->stk - amx->hea - numnatives * sizeof( unsigned int));
  if( list )
  {
    for( i=0 ; list[ i ].name != NULL && ( i < number || number == -1 ); i++ ){
      hash_tbl[ i ] = CalcHash( list[ i ].name );
    }
  }
  ...
  for( i=0 ; i < numnatives ; i++ )
  {
    if( func->address == 0 )
    {
      ...
      funcptr = ( list != NULL ) ? findfunction( GETENTRYNAME( hdr, func ), list, number, hash_tbl ): NULL;
      if( funcptr != NULL )
        func->address = ( ucell )funcptr;
      else{// GameFreak for Debug
        /*
        -------------------------------------------------------------
        これは、Pawn version 3.3.4097に
        株式会社ゲームフリークが改変を加えたものです。
        ...
        */
        ...
        if( !list )
        {
#ifdef GFL_PAWN_USE_HASH
          {
  #if defined(GF_PLATFORM_WIN32)
            TCHAR buf[ 16 ];
            PRINT_LOG( L"\nError NotFound [" );
            wsprintf( buf,L"0x%08x", GETENTRYNAME( hdr, func));
            PRINT_LOG( buf );
           PRINT_LOG( L"]\n" );
  #else
            char buf[ 16 ];
            PRINT_LOG( "\nError NotFound [" );
            sprintf( buf,"0x%08x", GETENTRYNAME( hdr, func));
            PRINT_LOG( buf );
            PRINT_LOG( "]\n" );
  #endif
          }
#else
          PRINT_LOG( GETENTRYNAME( hdr, func));
          PRINT_LOG( L"]\n" );
#endif
        }
      }
...
static void amx_UpdateStackHeap(AMX* amx)
{
  // GameFreak add this function for memory usage check
  size_t stack, heap;
  stack = amx->stp - amx->stk;
  heap = amx->hea - amx->hlw;
  if((s32)amx->userdata[1] < (s32)stack){
      amx->userdata[1] = (void*)stack;
  }
  if((s32)amx->userdata[2] < (s32)heap){
      amx->userdata[2] = (void*)heap;
  }
}
...
#define CHKMARGIN()     amx_UpdateStackHeap(amx); if (hea+STKMARGIN>stk) return AMX_ERR_STACKERR
...
static const void * const amx_opcodelist[] = {
  ...
  op_align_pri:
    GETPARAM(offs);
    #if BYTE_ORDER==PAWN_LITTLE_ENDIAN
