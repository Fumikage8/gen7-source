...
protected:
  ...
  const void* GetProperty( const c8 * const name ) const {
    ...
    u32 ofs = gfl2::core::Utility::CalcHash( name );
    u32 add = (ofs << 1) | 1;
    ...
    for( u32 i = 0; i <= mask; i++ ){
      ...
      if( tbl && 0 == gfl2::core::Utility::Strcmp( name, PropertyBase::GetPropertyName( tbl ) ) ){
        return tbl;
      }
    }
    return 0;
...
public:
  ...
  void SetPropertyListHeap( gfl2::heap::HeapBase* heap ){
