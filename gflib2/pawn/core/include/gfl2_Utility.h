...
public:
  ...
  static T AlignAddress( T val, S size ) {
  ...
  template<typename T, typename S>
  static T AddAddress( T adr, S val ) {
  ...
  template<typename T, typename S>
  static T SubAddress( T adr, S val ) {
    return reinterpret_cast<T> ((size_t) adr - (size_t) val);
  }
   ...
   * パディングを入れる
     ...
     */
  template<typename T>
  static T* PaddingAddress( T* dat, u32 aln ){
  ...
  static void Memcpy( void* out, const void* in, size_t size );
  ...
  static void Memmove( void* out, const void* in, size_t size );
  static void Memset( void* out, const int val, size_t size );
  static s32 Strcmp( const c8 * const str1, const c8 * const str2 );
  static s32 Strncmp( const c8 * const str1, const c8 * const str2, const size_t size );
