 ...
 */
//------------------------------------------------------------------
TaskSystem::TaskSystem( gfl2::heap::HeapBase* sysHeap , gfl2::heap::HeapBase* userHeap , const u32 taskMax , const u32 workSize )
:mTaskMax(taskMax)
,mWorkSize(workSize)
,mHeap(userHeap)
,mAddFlag(false)
...
,mTaskNum(0)
,isSyncMode(false)
,mLastCreatedTask(NULL)
{  
  Task *now;

  //4バイト境界に適合するように正規化する
  mWorkSize = gfl2::math::RoundUp(mWorkSize,4);
  mTcbArray = GflHeapAllocMemory( sysHeap , (sizeof(Task)+mWorkSize)*mTaskMax );
  
  for (u32 i = 0; i < mTaskMax; i++)
  {
    now = GetTaskPtr(i);
 ...
 */
//------------------------------------------------------------------
void TaskSystem::Update(int step)
{  
  ...
  {
    mSuicideFlag = false;
    mNowChain->mFunc(mNowChain, mNowChain->mWork,step);
...
*/
//------------------------------------------------------------------------------
Task* TaskSystem::Create( TASKFUNC * func, u32 workSize, u32 pri)
{
  Task* get;
  Task* find;
  Task* head = &(mTtcbFirst);
  ...
  if (workSize <= mWorkSize)
  {
    ...
    gfl2::std::MemClear(get->mWork,workSize);
...
*/
//------------------------------------------------------------------------------
void Task::Update(int step)
{
  if(step>0)
    mFunc(this,mWork,step);
}
