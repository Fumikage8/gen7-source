//======================================================================
/**
 * @file    StatusDrawerBase.cpp
 * @date    2015/11/27 17:46:55
 * @author  fukushima_yuya
 * @brief   ポケモンステータス：描画共通処理
 * @par Copyright
...
//------------------------------------------------------------------------------
  ...
  //----------------------------------------------------------------------------
  /**
   * @brief   子ペインの追加(技入れ替え専用)
   * @param   plateIndex    対象プレートインデックス
   * @param   appendType    追加の仕方の指定
   */
  //----------------------------------------------------------------------------
  void DrawerBase::AppendChildPane( u8 plateIndex, u8 appendType )
  {
    app::util::G2DUtil* pG2D = GetG2D();

    gfl2::lyt::LytPaneIndex parentIndex = PANENAME_FIELDSTATUS_MIN_LOW_000_PANE_ALL_EGG;
    gfl2::lyt::LytPaneIndex childIndex  = PANENAME_FIELDSTATUS_MIN_LOW_000_PANE_PARTSSKILLPLT_00 + plateIndex;
    gfl2::lyt::LytPaneIndex offsetIndex = 0;
    u32 mode = 0;
    ...
    case 0:
      {
        offsetIndex = PANENAME_FIELDSTATUS_MIN_LOW_000_PANE_NULL_00;
        mode = app::util::G2DUtil::APPEND_CHILD_MODE_INSERT;
      } break;

    case 1:
      {
        offsetIndex = PANENAME_FIELDSTATUS_MIN_LOW_000_PANE_NULL_01;
        mode = app::util::G2DUtil::APPEND_CHILD_MODE_INSERT;
      } break;

    case 2:
      {
        offsetIndex = PANENAME_FIELDSTATUS_MIN_LOW_000_PANE_NULL_02;
        mode = app::util::G2DUtil::APPEND_CHILD_MODE_INSERT;
        //mode = app::util::G2DUtil::APPEND_CHILD_MODE_BOTTOM;
      } break;
    }

    pG2D->AppendChild( m_LayoutID, parentIndex, childIndex, mode, offsetIndex );

    static const char* c_PlateName[] = {
      "SKILLPLT_00",
      "SKILLPLT_01",
      "SKILLPLT_02",
      "SKILLPLT_03",
    };
    static const char* c_ParentName[] = {
      "PlateLow",
      "PlateHigh",
      "PlateMiddle",
    };

    u32 parentID = offsetIndex - PANENAME_FIELDSTATUS_MIN_LOW_000_PANE_NULL_00;
    FUKUSHIMA_PRINT( "PaneName[%s] : Insert[%s]\n", c_PlateName[plateIndex], c_ParentName[parentID] );
  }
...
//------------------------------------------------------------------------------
  ...
  /**
   * @brief   リソースIDの取得
   * return   gfl2::lyt::LytMultiResIDクラス
   */
  //----------------------------------------------------------------------------
  gfl2::lyt::LytMultiResID& DrawerBase::GetLytMultiResID( void )
  {
    return m_ResID;
  }
...
//------------------------------------------------------------------------------
   ...
   */
  //------------------------------------------------------------------
  bool MenuCursor::IsVisible( void )
  {
    if( !m_pMenuCursor )
    {
      GFL_ASSERT(0);
    ...
    return m_pMenuCursor->IsVisible();
  }
