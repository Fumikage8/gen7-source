//======================================================================
/**
 * @file WazaOshieProc.cpp
 * @date 2015/07/03 11:26:55
...
static const u32 SIZE_MY_APPDEV     = 2 * 1024 * 1024;    //!< 2Mバイト
 ...
 */
//------------------------------------------------------------------------------
void Proc::CreateWazaOshieFrame( void )
{
  ...
  // セットアップパラメータ
  WazaOshieFrame::SetupParam setupParam;
  {
    setupParam.heap           = m_pAppHeap;
    setupParam.renderMan      = m_pAppRenderingManager;
    setupParam.fadeMan        = GFL_SINGLETON_INSTANCE(gfl2::Fade::FadeManager);
    setupParam.cmnIcon        = m_pCommonIcon;
    setupParam.createNumFont  = true;

    m_PartyData = GFL_NEW( m_pAppHeap->GetDeviceHeap() ) App::PokeList::IntermediateData();
    setupParam.partyData = m_PartyData;
    {
      GFL_ASSERT( setupParam.partyData );

      if( m_pAppParam->partyData == NULL )
      {
        setupParam.partyData->SetParameter( gd->GetPlayerParty() );
      }
      else {
        setupParam.partyData->SetParameter( m_pAppParam->partyData );
      }
    }
  }

  // フレームの生成
  m_pWazaOshieFrame = MENU_FRAME_CALL_MODAL<applib::frame::Manager, WazaOshie::WazaOshieFrame>( this->GetFrameManager() );
  {
    GFL_ASSERT( m_pWazaOshieFrame );
    
    m_pWazaOshieFrame->Setup( m_pAppParam, setupParam );
  }
}
