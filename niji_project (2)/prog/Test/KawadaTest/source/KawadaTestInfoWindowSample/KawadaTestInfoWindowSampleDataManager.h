#if !defined(NIJI_PROJECT_TEST_KAWADATESTINFOWINDOWSAMPLEDATAMANAGER_H_INCLUDED)
#define NIJI_PROJECT_TEST_KAWADATESTINFOWINDOWSAMPLEDATAMANAGER_H_INCLUDED
#pragma once


//==============================================================================
/**
 @file    KawadaTestInfoWindowSampleDataManager.h
...
class KawadaTestInfoWindowSampleDataManager
{
  GFL_FORBID_COPY_AND_ASSIGN(KawadaTestInfoWindowSampleDataManager);

public:
  //! @brief  データID
  enum DataId
  {
    DATA_ID_SIMPLE_LAYOUT_TEST_APPLYT  = 0,
    DATA_ID_MY_DEBUG_MESSAGE_DAT       = 1,
...
public:
  ...
  KawadaTestInfoWindowSampleDataManager(app::util::Heap* appHeap, gfl2::fs::AsyncFileManager* romAsyncFileManager);
  virtual ~KawadaTestInfoWindowSampleDataManager();
...
private:
  ...
  u32                          m_step;
  DataId                       m_dataId;    // ロード中のデータIDが入っている。ArcFileをオープンしているときはこれから読み込むべきデータIDが入っている。ArcFileをクローズしているときは読み終わったデータIDが入っている。
  gfl2::fs::ArcFile::ARCID     m_arcId;     // m_dataIdから決められる。  // オープン中のARCIDが入っている。ArcFileをオープンする前はこれからオープンすべきARCIDが入っている。ArcFileをクローズしているときはクローズ中のARCIDが入っている。
  gfl2::fs::ArcFile::ARCDATID  m_arcDatId;  // m_dataIdから決められる。  // ロード中のARCDATIDが入っている。ArcFileをオープンしているときはこれから読み込むべきARCDATIDが入っている。ArcFileをクローズしているときは読み終わったARCDATIDが入っている。
...
#endif  // NIJI_PROJECT_TEST_KAWADATESTINFOWINDOWSAMPLEDATAMANAGER_H_INCLUDED
