//==============================================================================
/**
 @file    KawadaTestInfoWindowSampleMain.cpp
...
#include "KawadaTestInfoWindowSampleMain.h"
...
KawadaTestInfoWindowSampleMain::KawadaTestInfoWindowSampleMain(GameSys::GameManager* gameManager, gfl2::heap::HeapBase* parentHeapMem, gfl2::heap::HeapBase* parentDevMem)
...
KawadaTestInfoWindowSampleMain::~KawadaTestInfoWindowSampleMain()
{
  this->terminate();
}

void KawadaTestInfoWindowSampleMain::Update(void)
{
  // データマネージャ
...
void KawadaTestInfoWindowSampleMain::PreDraw(void)
{
...
void KawadaTestInfoWindowSampleMain::Draw(gfl2::gfx::CtrDisplayNo displayNo)
{
  if(m_step == STEP_WORKING)  // このテストでは、ワーキングステップのときだけ描画する
  {
    // フレーム
...
void KawadaTestInfoWindowSampleMain::initialize(GameSys::GameManager* gameManager, gfl2::heap::HeapBase* parentHeapMem, gfl2::heap::HeapBase* parentDevMem)
{
...
void KawadaTestInfoWindowSampleMain::terminate(void)
{
  if(m_enableToEnd)
  {
    m_uniMem = NULL;
...
void KawadaTestInfoWindowSampleMain::changeStep(Step step)
{
  m_step      = step;
  m_stepCount = 0;
}

void KawadaTestInfoWindowSampleMain::updateFunc_None(void)
{}
void KawadaTestInfoWindowSampleMain::updateFunc_Create(void)
{
  switch(m_stepCount)
  {
  case 0:
    {
      ...
      static const s32 HEAP_SIZE_APP_DEV        = 0xFF0000;//= 0x800000;//= 0xF00000;  //!< このアプリのapp::util::Heap
      ...
      // データマネージャ
      m_dataManager = GFL_NEW(m_uniMem) KawadaTestInfoWindowSampleDataManager(m_appHeap, m_gameManager->GetAsyncFileManager());
      m_dataManager->Start();
      ...
      m_frameListener = GFL_NEW(m_uniMem) KawadaTestInfoWindowSampleFrameListener();
  ...
  case 3:
    {
      ...
      // フレーム生成
      m_frameCell = MENU_FRAME_CALL_MODAL<applib::frame::Manager, KawadaTestInfoWindowSampleFrameCell>(m_frameManager);
      KawadaTestInfoWindowSampleFrameCell::Param param;
...
void KawadaTestInfoWindowSampleMain::updateFunc_Working(void)
{
  // 描画時は必ず行う更新
  this->updateForDraw();
...
void KawadaTestInfoWindowSampleMain::updateFunc_Destroy(void)
{
  switch(m_stepCount)
  {
  case 0:
...
void KawadaTestInfoWindowSampleMain::updateForDraw(void)
{
  // 特にやることはない
}
