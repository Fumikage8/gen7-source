//==============================================================================
/**
 @file    KawadaTestLayoutSampleDataManager.cpp
...
KawadaTestLayoutSampleDataManager::KawadaTestLayoutSampleDataManager(app::util::Heap* appHeap, gfl2::fs::AsyncFileManager* romAsyncFileManager)
  : m_appHeap(appHeap),
    m_romAsyncFileManager(romAsyncFileManager),
    m_state(STATE_STOP),
    m_seq(SEQ_START_OPEN),
    m_step(0),
    m_dataIdOfSeqLoad(DATA_ID_SIMPLE_LAYOUT_TEST_APPLYT),
    m_dataLoadBitFlag(0)
{
...
KawadaTestLayoutSampleDataManager::~KawadaTestLayoutSampleDataManager()
{
  if( !(this->CanDestroy()) )
  {
    // 破棄してはいけない状態のとき
...
bool KawadaTestLayoutSampleDataManager::CanDestroy(void) const
{
  return (m_state == STATE_END);  // @todo 通信エラーが途中で起こり得るなら、中断にも対応できるようにしたい。
}

void KawadaTestLayoutSampleDataManager::Update(void)
{
...
void KawadaTestLayoutSampleDataManager::Start(void)
{
  if(
       m_state == STATE_STOP
...
void KawadaTestLayoutSampleDataManager::Stop(void)
{
  if(
       m_state == STATE_WORK
  )
...
void* KawadaTestLayoutSampleDataManager::GetDataBuf(DataId dataId) const
{
...
void KawadaTestLayoutSampleDataManager::changeSeq(Seq nextSeq)
{
  m_seq = nextSeq;
  m_step = 0;
}

void KawadaTestLayoutSampleDataManager::update_StartOpen(void)
{
...
void KawadaTestLayoutSampleDataManager::update_WaitOpen(void)
{
  gfl2::fs::ArcFile::ARCID    arcId    = gfl2::fs::ArcFile::ARCID_NULL;
  gfl2::fs::ArcFile::ARCDATID arcDatId = gfl2::fs::ArcFile::ARCDATID_NULL;
  this->getArcInfoOfDataId(m_dataIdOfSeqLoad, &arcId, &arcDatId);
...
void KawadaTestLayoutSampleDataManager::update_StartLoad(void)
{
  gfl2::fs::ArcFile::ARCID    arcId    = gfl2::fs::ArcFile::ARCID_NULL;
  gfl2::fs::ArcFile::ARCDATID arcDatId = gfl2::fs::ArcFile::ARCDATID_NULL;
  this->getArcInfoOfDataId(m_dataIdOfSeqLoad, &arcId, &arcDatId);
...
void KawadaTestLayoutSampleDataManager::update_WaitLoad(void)
{
...
void KawadaTestLayoutSampleDataManager::update_StartClose(void)
{
  gfl2::fs::ArcFile::ARCID    arcId    = gfl2::fs::ArcFile::ARCID_NULL;
  gfl2::fs::ArcFile::ARCDATID arcDatId = gfl2::fs::ArcFile::ARCDATID_NULL;
  this->getArcInfoOfDataId(m_dataIdOfSeqLoad, &arcId, &arcDatId);
...
void KawadaTestLayoutSampleDataManager::update_WaitClose(void)
{
...
void KawadaTestLayoutSampleDataManager::update_End(void)
{
  // 何もしない
}

void KawadaTestLayoutSampleDataManager::getArcInfoOfDataId(const DataId a_dataId, gfl2::fs::ArcFile::ARCID* a_arcId, gfl2::fs::ArcFile::ARCID* a_arcDatId) const
{
  gfl2::fs::ArcFile::ARCID    arcId    = gfl2::fs::ArcFile::ARCID_NULL;
  gfl2::fs::ArcFile::ARCDATID arcDatId = gfl2::fs::ArcFile::ARCDATID_NULL;
  if(a_dataId == DATA_ID_SIMPLE_LAYOUT_TEST_APPLYT)
  {
    arcId    = NIJI_ARCID_NUM(ARCID_KAWADA_TEST_DATA);
    arcDatId = GARC_kawada_test_data_simple_layout_test_applyt_COMP;
  }
  else if(a_dataId == DATA_ID_SIMPLE_EFFECT_TEST)
  {
    arcId    = NIJI_ARCID_NUM(ARCID_KAWADA_TEST_DATA);
    //arcDatId = GARC_kawada_test_data_Gift_CardEmit_000_bptcl_COMP;
    arcDatId = GARC_kawada_test_data_ea_z_beads_bptcl_COMP;
  }
  *a_arcId    = arcId;
  *a_arcDatId = arcDatId;
}
