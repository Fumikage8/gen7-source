//==============================================================================
/**
 @file    KawadaTestPokeModelSampleEvent.cpp
...
void KawadaTestPokeModelSampleEvent::StartEvent(GameSys::GameManager* gmgr)
{
  GameSys::GameEventManager* pGameEventManager = gmgr->GetGameEventManager();
  GAMESYSTEM_EVENT_CALL<GameSys::GameEventManager, KawadaTestPokeModelSampleEvent>(pGameEventManager);
}


KawadaTestPokeModelSampleEvent::KawadaTestPokeModelSampleEvent(gfl2::heap::HeapBase* heap)
  : GameSys::GameEvent(heap),
    m_proc(NULL)
{}
KawadaTestPokeModelSampleEvent::~KawadaTestPokeModelSampleEvent()
{}

bool KawadaTestPokeModelSampleEvent::BootChk(GameSys::GameManager* /*gmgr*/)
{
  return true;
}
void KawadaTestPokeModelSampleEvent::InitFunc(GameSys::GameManager* /*gmgr*/)
{}
GameSys::GMEVENT_RESULT KawadaTestPokeModelSampleEvent::MainFunc(GameSys::GameManager* gmgr)
{
  enum
  {
    SEQ_WAIT_OTHER_PROC_END,
    SEQ_CALL_POKEMODEL_SAMPLE_PROC,
    SEQ_WAIT_POKEMODEL_SAMPLE_PROC_START,
    SEQ_WAIT_POKEMODEL_SAMPLE_PROC_END,
    SEQ_CALL_LAUNCHER,
    SEQ_END,
  };
  ...
  case SEQ_WAIT_OTHER_PROC_END:
    {
      ...
      {
        seq = SEQ_CALL_POKEMODEL_SAMPLE_PROC;
      }
      break;
    }
  case SEQ_CALL_POKEMODEL_SAMPLE_PROC:
    {
      // このイベントで扱うproc作成
      KawadaTestProcParam param;
      param.sampleId = KawadaTestProcParam::SAMPLE_ID_POKEMODEL_MAIN;
      m_proc = GameSys::CallKawadaTestProc(&param);
      seq = SEQ_WAIT_POKEMODEL_SAMPLE_PROC_START;
      break;
    }
  case SEQ_WAIT_POKEMODEL_SAMPLE_PROC_START:
    {
      if( gmgr->IsProcessExists() != false )
      {
        // このイベントで扱うprocが動作中
        seq = SEQ_WAIT_POKEMODEL_SAMPLE_PROC_END;
      }
      break;
    }
  case SEQ_WAIT_POKEMODEL_SAMPLE_PROC_END:
    {
      // 全てのprocがなくなるのを待つ
      if( gmgr->IsProcessExists() == false )
      {
...
void KawadaTestPokeModelSampleEvent::EndFunc(GameSys::GameManager* /*gmgr*/)
{}
