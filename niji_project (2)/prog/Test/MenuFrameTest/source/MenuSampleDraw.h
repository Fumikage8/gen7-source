...
#if !defined( __MENU_SAMPLE_DRAW_H__ )
#define	__MENU_SAMPLE_DRAW_H__
#pragma once
...
#include "MenuSampleDrawListener.h"
#include "AppLib/include/Util/app_util_heap.h"
...
/// タイトル用ビュー
class MenuSampleDraw : public app::ui::UIView, public app::ui::UIInputListener
{
  GFL_FORBID_COPY_AND_ASSIGN(MenuSampleDraw);
...
public:
   ...
   * @param heap  ヒープ
   */
//  MenuSampleDraw( app::util::Heap * heap,gfl2::lyt::LayoutRenderPath* pLayoutRenderPath );
  MenuSampleDraw( app::util::Heap * heap,app::util::AppRenderingManager* pAppRanderingManager );

  /// デストラクタ
  virtual ~MenuSampleDraw();
...
public:
  /// このビュー専用のUIリスナー設定
  void SetTitleViewUIListener( MenuSampleDrawListener *listener );
...
public:
  ...
  void ExpandPokemon(int no, int monsno );
...
private:
  ...
  MenuSampleDrawListener*        m_uiListener;     ///< UIイベントリスナー
