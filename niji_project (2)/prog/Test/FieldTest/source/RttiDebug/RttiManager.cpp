...
void RttiParamManager::getParamList( const type_info*                   targetClassInfo ,
                                     ...
                                     uint64_t                           baseOffset)
{
  if( _parameterManager.Size() < 1 )
  {
    return;
  }

  for ( gfl2::util::Map< const type_info*,gfl2::util::List<MemberProperty*>*>::Iterator ite = _parameterManager.Begin() ; ite != _parameterManager.End() ; ite++  )
  {
    ...
    if( pInfo == targetClassInfo )
    {
      ...
      for( gfl2::util::List<MemberProperty*>::Iterator iteListParam = pListParam->Begin() ; iteListParam != pListParam->End() ; iteListParam++ )
      {
        ...
        {
          RttiParamManager::MemberProperty* pNewMemProp = GFL_NEW(_pHeap) gfl2::rttidebug::RttiParamManager::MemberProperty();
          *pNewMemProp = *pParam;
  ...
  gfl2::util::Map< const type_info*,const type_info* >::Iterator ite = _relationManager.Find( targetClassInfo );
  if( ite != _relationManager.End() && ite->second != NULL )
  {
