...
public:
  ...
  */
  ...
  {
    _pHeap = pHeap;
    _parameterManager.CreateBuffer(pHeap,bufferSize);
    _relationManager.CreateBuffer(pHeap,bufferSize);
  }
  ...
  */
  //------------------------------------------------------------------
  inline void insertToParam( const type_info* pTypeInfo, gfl2::util::List<MemberProperty*>* pMemberPropertyList)
  { 
    _parameterManager.Insert( gfl2::util::Pair<const type_info*,gfl2::util::List<MemberProperty*>*>( pTypeInfo , pMemberPropertyList ) );
  }
  ...
  */
  //------------------------------------------------------------------
  inline void insertToReleation( const type_info* pClassTypeInfo, const type_info* pParentClassTypeInfo)
  { 
    _relationManager.Insert ( gfl2::util::Pair<const type_info*, const type_info* >( pClassTypeInfo , pParentClassTypeInfo ) ); 
  }
  ...
  */
  //------------------------------------------------------------------
  gfl2::heap::HeapBase* getHeap(void){ return _pHeap; }

private:

  // マネージャー全体で利用するヒープ
  gfl2::heap::HeapBase* _pHeap;

  // クラスのメンバ情報保持マネジャー
  gfl2::util::Map< const type_info*,gfl2::util::List<MemberProperty*>* > _parameterManager;   // first  : 型情報
                                                                                              // second : メンバ情報リスト
  // クラスの継承関係保持マネージャー
  gfl2::util::Map< const type_info*,const type_info* >                   _relationManager;    // first  : 検索キーとなるクラスの型情報
...
// クラスのメンバ情報登録終了
#define RTTI_END_REGIST(ClassName)                                                gfl2::rttidebug::RttiParamManager::getInstance()->insertToParam( &typeid( ClassName ) , pRttiParameterList );
