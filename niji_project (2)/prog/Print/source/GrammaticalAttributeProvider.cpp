...
uint GrammaticalAttributeProvider::getDatId(uint lang)
{
  ...
  static const u16 lang_to_grammar_garc_id[] = {
    ...
    GARC_grammar_Simp_Chinese_DAT,
    GARC_grammar_Trad_Chinese_DAT,
  };
...
uint GrammaticalAttributeProvider::getInitialSoundTypeFrench(const gfl2::str::STRCODE* sp)
{
  ...
  if (isVowelAlphabet(c)){  //最初の文字が母音
    return FRENCH_INITIALSOUNDTYPE_VOWEL;
  }
  if(c==0x152 || c==0x153){  //最初の文字が oe
    return FRENCH_INITIALSOUNDTYPE_VOWEL;
  }
  if(c==0xc6 || c==0xe6){  //最初の文字が ae
    return FRENCH_INITIALSOUNDTYPE_VOWEL;
  }

  if (c == 'y' || c == 'Y')     // 最初の文字が y / Y なら次の文字を見る
  {
...
//イタリア語の最初の音タイプ判定
//    最初の文字が母音字⇒母音
//    最初の文字が z, x ,y ⇒ s+子音、ｚ 等 (子音2) momijiで修正。先頭がYで２文字目がｙを含む母音だったとき母音発音
//      (以下3つは2文字以上の場合のみ)
//      最初の2文字が gn, pn, ps ⇒ s+子音、ｚ 等 (子音2)
//      最初の文字が h で、次の文字が存在してそれが母音字⇒母音　momijiで修正。最初の文字が hなら母音
//      最初の文字が s で、次の文字が存在してそれが母音字（Yを含む）ではない⇒ s+子音、ｚ (子音2)　momijiでは２文字目チェックをYを含む母音チェックに変更
//    それ以外⇒子音（その他）
uint GrammaticalAttributeProvider::getInitialSoundTypeItalian(const gfl2::str::STRCODE* sp)
{
  ...
  else if (c1 == 'z' || c1 == 'Z' || c1 == 'x' || c1 == 'X' || c1 == 'y' || c1 == 'Y')
  {
    if (c1 == 'y' || c1 == 'Y')
    {
      if ((c2 != gfl2::str::EOM_CODE) && (!isVowelAlphabetPlusY(c2)))//momijiで「y」のみ２文字目母音チェック（ｙ含む）を追加
      {
        return ITALIAN_INITIALSOUNDTYPE_VOWEL;
      }
    }
  ...
  else if ((c1 == 'h' || c1 == 'H') ) //momijiで修正。一文字目が「ｈ」なら例外なく母音発音でよい
  {
    return ITALIAN_INITIALSOUNDTYPE_VOWEL;
  }
  else if ((c1 == 's' || c1 == 'S') && !isVowelAlphabetPlusY(c2))//momijiで２文字目母音チェックにYを含めるように修正
  {
    return ITALIAN_INITIALSOUNDTYPE_CONS2;
  }
  else
...
// 指定した文字が母音のアルファベットかもしくはYかどうかを得る
bool GrammaticalAttributeProvider::isVowelAlphabetPlusY(gfl2::str::STRCODE c)
{
  if ((c == 'y' || c == 'Y'))
  {
    return true;
