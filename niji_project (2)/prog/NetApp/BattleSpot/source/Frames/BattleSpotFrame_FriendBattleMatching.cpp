//==============================================================================
/**
 * @file   BattleSpotFrame_FriendBattleMatching.cpp
...
#include  "./BattleSpotFrame_FriendBattleMatching.h"
...
//------------------------------------------------------------------
/**
  *  @func     Frame_FriendBattleMatching
  *  @brief    ctor
  */
//------------------------------------------------------------------
Frame_FriendBattleMatching::Frame_FriendBattleMatching(BattleSpotWork* pWork)
...
/**
  ...
  */
//------------------------------------------------------------------
Frame_FriendBattleMatching::~Frame_FriendBattleMatching()
{
#if defined(GF_PLATFORM_CTR)
  if( m_rankingClient )
  {
...
/**
  ...
  */
//------------------------------------------------------------------
bool  Frame_FriendBattleMatching::startup()
{
  ...
  /*  views  */
  {
    ...
    m_pUpperView->SetViewMode(UpperView_Information::VIEWMODE_FriendMatch_Matching);
...
/**
  ...
  */
//------------------------------------------------------------------
void  Frame_FriendBattleMatching::setListener()
{
...
/**
  ...
  */
//------------------------------------------------------------------
void  Frame_FriendBattleMatching::removeListener()
{
...
/**
  ...
  */
//------------------------------------------------------------------
void  Frame_FriendBattleMatching::updateSequence()
{
  switch(m_State)
  {
    case  STATE_Entry:
      {
        GameSys::GameData  *game_data = GFL_SINGLETON_INSTANCE( GameSys::GameManager )->GetGameData();
        if( m_pWork->m_pBattleSpotSave->GetFriendMatchPID() == game_data->GetPlayerStatus()->GetPrincipalId() )
        {//PID一致
    ...
    case  STATE_Matching:
      {
        ...
        {//マッチング確定
          ...
          m_pLowerView->SetViewMode(LowerView_Card::VIEWMODE_FriendMatch_Matched);
          m_pUpperView->SetViewMode(UpperView_Information::VIEWMODE_FriendMatch_Matched);
    ...
    case  STATE_Ready:
        ...
        {//バトルへ
          ...
          //対戦回数＋１
          m_pWork->m_pBattleSpotSave->SetFriendMatchCount( m_pWork->m_pBattleSpotSave->GetFriendMatchCount() + 1 );

          exitFrame(RESULT_FriendBattleMatching_Next);
        }
      break;
...
/**
  ...
  */
//------------------------------------------------------------------
bool  Frame_FriendBattleMatching::cleanup()
{
  bool  bAllDone = true;
...
void  Frame_FriendBattleMatching::LowerView_Card_OnEvent(LowerView_Card* pSender, const LowerView_Card::IEventHandler::EventCode eventCode)
{
  switch( eventCode )
  {
  case LowerView_Card::IEventHandler::EVENT_Message_EOM:
      ...
      {
        ExitSelf(RESULT_FriendBattleMatching_Timeout);
      }
   break;
  }
}
...
void  Frame_FriendBattleMatching::LowerView_Selection_OnEvent(LowerView_Selection* pSender, const LowerView_Selection::IEventHandler::EventCode eventCode)
{
  switch(eventCode)
  {
  case LowerView_Selection::IEventHandler::EVENT_Message_EOM:
    {
       ExitSelf(RESULT_FriendBattleMatching_InvalidPid);
    }
  break;
  }
}
...
void Frame_FriendBattleMatching::OnPacketTransferSuccess( NetLib::P2P::P2pPacketTransfer* pP2pPacketTransfer )
{
#if defined(GF_PLATFORM_CTR)
  ...
  GFL_PRINT("Frame_FriendBattleMatching::OnPacketTransferSuccess : data count[%d]\n",receivedDataCount);
...
/**
  ...
  */
//------------------------------------------------------------------
void  Frame_FriendBattleMatching::ExitSelf(const FrameResult frameResult)
{
...
/**
  ...
  */
//------------------------------------------------------------------
bool Frame_FriendBattleMatching::UpdateMatching()
{
#if defined(GF_PLATFORM_CTR)
  NetLib::P2P::P2pConnectionManager* pP2pConnectionManager = GFL_SINGLETON_INSTANCE(NetLib::P2P::P2pConnectionManager);
  ...
  case MATCHING_INIT:
    {
      ...
      GFL_PRINT("Frame_FriendBattleMatching : CutRate : %d %d %d \n", cut_per, cut_per_min, cut_per_max );

      NetLib::P2P::ENUM_NIJI_P2P_ID p2pID = NetLib::P2P::NIJI_P2P_WIFI_CUP;
      u32 category_id = m_pWork->m_pAppParam->m_pRegulationDrawInfo->GetRegulation().GetValueParam( Regulation::DISCRIMINANT_ID );

      u32 QRParam1 = GFL_SINGLETON_INSTANCE( GameSys::GameManager )->GetGameData()->GetQRParam1();
      u32 QRParam2 = GFL_SINGLETON_INSTANCE( GameSys::GameManager )->GetGameData()->GetQRParam2();
      ...
      gflnet2::nex::NexMatchMakingRule rule3( 3 , QRParam1 , QRParam1 , QRParam1 , gflnet2::nex::NexMatchMakingRule::VALUE_TYPE_CREATE_ONLY );
      gflnet2::nex::NexMatchMakingRule rule4( 3 , QRParam2 , QRParam2 , QRParam2 , gflnet2::nex::NexMatchMakingRule::VALUE_TYPE_SEARCH_ONRY );

      list.push_back( &rule0 );
      list.push_back( &rule1 );
      list.push_back( &rule2 );
      list.push_back( &rule3 );
      list.push_back( &rule4 );
  ...
  case MATCHING_WAIT:
    {
      ...
      {
        //アイコンセットアップ開始
        m_pUpperView->StartIconSetup(UpperView_Information::VIEWMODE_FriendMatch_Matched);
  ...
  case DISCONNECT_WAIT:
    {
      if( pP2pConnectionManager->IsDisconnectSuccess() )
      {
        ...
        {
          m_pLowerView->SetViewMode(LowerView_Card::VIEWMODE_FriendMatch_MatchingFailed);
...
bool Frame_FriendBattleMatching::ReadyWait()
{
  if( s_wait++ > 30 )
  {
    s_wait = 0;
...
void Frame_FriendBattleMatching::UpdateRankData()
{
#if defined(GF_PLATFORM_CTR)
  switch( m_seq ){
  ...
  case RANK_SEQ_END:                ///< 終了
    ...
    /*if( m_common_data->select_mode == SELECT_INTERNET_CUP )
    {
      ...
      // サーバから取得した対戦回数がセーブしてある対戦回数を上回っていると書き換える
      {
        ...
        if( battle_count > sv_battle_match->GetFriendMatchCount() )
        {
          sv_battle_match->SetFriendMatchCount( battle_count );
    ...
    m_pViewContainer->SetScene(BattleSpotViewContainer::SCENE_FriendMatch_Matching);
    m_pLowerView->SetViewMode(LowerView_Card::VIEWMODE_FriendMatch_Matching);
 ...
 */
//--------------------------------------------------------------------------
bool Frame_FriendBattleMatching::InitializeRanking( void )
{
#if defined(GF_PLATFORM_CTR)
 ...
 */
//--------------------------------------------------------------------------
bool Frame_FriendBattleMatching::FinalizeRanking( void )
{
#if defined(GF_PLATFORM_CTR)
  if( m_rankingClient )
  {
 ...
 */
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnNexRankingUploadScoreSuccess( const u32 category )
{
 ...
 */
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnNexRankingDeleteScoreSuccess( const u32 category )
{
 ...
 */
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnNexRankingUploadCommonDataSuccess( void )
{
 ...
 */
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnNexRankingGetCommonDataSuccess( const nn::nex::qVector<qByte>& data )
{
 ...
 */
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnNexRankingGetListDataSuccess( const nn::nex::RatingRankData& data )
{
  ...
  {//相手の成績
    ...
    u32 todayLimitNum = m_pWork->m_pAppParam->m_pRegulationDrawInfo->GetRegulation().GetValueParam(Regulation::BATTLE_NUM_LIMIT);
    ...
    else
    {
      ...
      bool ret = m_pglConnector->DownloadBattleBox( otherNUID , NetLib::Pgl::PGLConnector::BATTLE_TYPE_FRIEND );
      if( ret == false )
      {
        pP2pConnectionManager->DisconnectStart(true);
        m_matchingState = DISCONNECT_WAIT;
  ...
  {//自分の成績
    ...
    if( battleCount > m_pWork->m_pBattleSpotSave->GetFriendMatchCount() )
    {
      m_pWork->m_pBattleSpotSave->SetFriendMatchCount( battleCount );//対戦回数保存
    }
 ...
 */
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnNexRankingClientError( const nn::nex::qResult& result, u32 errorCode )
{
...
*/
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnPGLDownloadBattleBoxSuccess( const u8* boxData, u32 dataSize )
{
...
*/
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnPGLConnectError( const NetLib::Pgl::PGL_COMMAND_TYPE commandType, const s32 httpStatus )
{
  NetLib::Error::NijiNetworkError error;
...
*/
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnPGLConnectError( const NetLib::Pgl::PGL_COMMAND_TYPE commandType, const NetLib::Pgl::PGL_COMMON_RESULT resultCode )
{
  NetLib::Error::NijiNetworkError error;
...
*/
//------------------------------------------------------------------
void Frame_FriendBattleMatching::OnPGLConnectError( const NetLib::Pgl::PGL_COMMAND_TYPE commandType, const nn::nex::qResult &result, u32 errorCode )
{
  NetLib::Error::NijiNetworkError error;
...
/**
  ...
  */
//------------------------------------------------------------------
bool Frame_FriendBattleMatching::OnSuspendWithHomeSleepAndPower( void )
{
#if defined(GF_PLATFORM_CTR)
...
void Frame_FriendBattleMatching::OnConfirmPlayStatusSuccess()
{
#if defined(GF_PLATFORM_CTR)
  GameSys::GameData  *game_data = GFL_SINGLETON_INSTANCE( GameSys::GameManager )->GetGameData();
  ...
  {//サインインずみ
    ...
    GFL_PRINT("todayLimitNum[%u]\n",todayLimitNum);

    if( serverTime != qrTime )
    {//終了後
      m_pWork->m_pAppParam->m_teamSelected = true;//強制的にフロー変える
      ExitSelf(RESULT_FriendBattleMatching_CupIsOver);
    }
    else if( m_pWork->IsRegulationTimeOver( dateTime.GetHour() ) )
    {//仲間大会は1日限定なので、時間過ぎてたら終わり扱い
      m_pWork->m_pAppParam->m_teamSelected = true;//強制的にフロー変える
      ExitSelf(RESULT_FriendBattleMatching_CupIsOver);
    }
    else if( m_pWork->IsRegulationTimeOut( dateTime.GetHour() ) )
    {//対戦できない時間
      m_pViewContainer->m_pLowerView_Selection->SetViewMode(LowerView_Selection::VIEWMODE_Message_NotAvailableTime);
      m_State = STATE_MessageWait;
    }
    else if( m_pWork->m_pBattleSpotSave->GetFriendMatchCount() >= todayLimitNum || 
      m_pWork->m_pBattleSpotSave->GetFriendMatchCount() >= m_pWork->m_pAppParam->m_pRegulationDrawInfo->GetRegulation().GetValueParam(Regulation::BATTLE_NUM) )
    {//対戦回数オーバー
...
void Frame_FriendBattleMatching::OnConfirmPlayStatusError( u32 errorCode )
{
  ExitSelf();
}
