 ...
 * @caution PM_LANG定義（PokemonVersion.hのLANG_〜）とは違い、
 * @caution かな・漢字の判別も入っている。
 * @caution 言語の判別に使用しないこと。
 */
//-----------------------------------------------------------------------------
enum MSGLANGID{
  MSGLANGID_KANA = 0,
...
/**
 * @brief 言語IDの設定
 * @param[in]   lang_id   言語指定ID（POKEMON_LANG_〜,include/PokemonVersion.hを参照）(例:POKEMON_LANG_JAPAN)
 * @param[in]   pTempHeap 作業用ヒープID(256KB=0x40000必要）
 *
 * @caution   内部で下記SetupLangResourceを呼び出す。みだりに使わないこと！！
 */
//-----------------------------------------------------------------------------
 ...
 * @caution  ゲーム内の言語ID→本体言語コードに変換するので、本体情報ではない
 */
//-----------------------------------------------------------------------------
...
public:
  ...
  /**
   * @brief 言語IDを取得する
   * @return  u8 言語ID（POKEMON_LANG_〜,include/PokemonVersion.hを参照）(例:POKEMON_LANG_JAPAN)
   */
  //--------------------------------------------------------------
  ...
  //--------------------------------------------------------------
  /**
   * @brief 言語ID→アーカイブ言語IDへの変換
   */
  //--------------------------------------------------------------
  static gfl2::fs::ToolDefine::ArcLangType::Tag ConvertPM2ARC( u8 lang_id );
...
private:
   ...
   */
  //--------------------------------------------------------------
  u8 m_LangId;  // 言語ID（POKEMON_LANG_〜,include/PokemonVersion.hを参照）(例:POKEMON_LANG_JAPAN)
};
