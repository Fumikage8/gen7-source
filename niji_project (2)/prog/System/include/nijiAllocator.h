...
public:
  ...
  static const u32 RESIDENT_VRAMB_SIZE = 0x180000;  //!常駐VRAMB　使用サイズ
...
public:
   ...
   * @param MallocMode  VRAM確保モード(基本はVRAM_MALLOC_AB)
   *
   * controlTypeで、RESIDENTを使用するのはGflUseで生成される描画エンジンのみです。
   * その他のシステムでは、APPLICATIONを使用しRESIDENTは使用しないでください。
   */
  nijiAllocator( gfl2::heap::HeapBase *devHeap, VramControlType controlType, VramMallocMode mode = VRAM_MALLOC_AB );
  ...
  */
  ...
  virtual void* LocalMemoryMalloc(gfl2::gfx::Pool allocation, u32 size, u32 alignment);

  //! VRAMからメモリ確保     内部コントロールタイプ
...
class nijiResidentAllocator : public nijiAllocator
{
public:
  nijiResidentAllocator( gfl2::heap::HeapBase *devHeap, VramControlType controlType, VramMallocMode mode = VRAM_MALLOC_AB );
  nijiResidentAllocator( gfl2::heap::HeapBase *devHeap );
  virtual ~nijiResidentAllocator();
