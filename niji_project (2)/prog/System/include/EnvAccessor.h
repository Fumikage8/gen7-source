...
public:
  ...
  enum AnimationBit
  {
    ...
    ANIMATION_BIT_FOG     = 1 << ANIMATION_TYPE_FOG,        ///< フォグ
    ANIMATION_BIT_BLOOM   = 1 << ANIMATION_TYPE_BLOOM       ///< ブルーム
  };
...
public:
   ...
   */
  void ChangeAnimationSmooth( gfl2::renderingengine::scenegraph::resource::ResourceNode* pMotionResourceRootNode, u32 slotIndex, AnimationBit animationBit, u32 smoothFrame);
  ...
  bool IsAnimationSmooth( AnimationBit animationBit) const;

private:

  gfl2::animation::AnimationSlotBase* GetAnimationSlot( u32 slotIndex, AnimationBit animationBit, bool isSmoothSlot);
  gfl2::animation::AnimationSlotBase* GetAnimationSlot( u32 slotIndex, AnimationType animationType, bool isSmoothSlot);
  f32 CalculateAllTimeAnimationFrame( f32 frameMax, s32 hour, s32 minute, s32 second) const ;

  template <typename F, typename A0>
  void SetAnimationForeach( F func, A0 arg0, u32 slotIndex, AnimationBit animationBit);

  void RequestAnimationSmooth( AnimationBit animationBit, s32 smoothFrame);
