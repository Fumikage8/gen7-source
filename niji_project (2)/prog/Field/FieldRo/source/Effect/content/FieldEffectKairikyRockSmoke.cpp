//======================================================================
/**
 * @file FieldEffectKairikyRockSmoke.cpp
 * @date 2015/12/09 13:09:56
 ...
 *  @brief  コンストラクタ
 */
EffectKairikyRockSmoke::EffectKairikyRockSmoke( void )
: IEffectBase()
, m_pEffectHeap( NULL )
 ...
 *  @brief  初期化
 */
void EffectKairikyRockSmoke::Initialize( SetupData &setupData )
{
 ...
 *  @brief  破棄
 */
bool EffectKairikyRockSmoke::Terminate( void )
{
  EndKill();
 ...
 *  @brief  再生
 */
void EffectKairikyRockSmoke::Start( void )
{
  if( !IsAlive() )
  {
    m_pEffectSystem->CreateEmitterSetID( m_pEffectHandle, m_pParent->GetPosition(), 0, EFFECT_RESOURCE_KAIRIKY_ROCK_SMOKE );
  }
}
 ...
 *  @brief  停止
 */
void EffectKairikyRockSmoke::EndKill( void )
{
  if( IsAlive() )
  {
 ...
 *  @brief  停止(フェード)
 */
void EffectKairikyRockSmoke::EndFade( void )
{
  if( IsAlive() )
  {
 ...
 *  @brief  再生確認
 */
bool EffectKairikyRockSmoke::IsAlive( void )
{
 ...
 *  @brief  更新処理
 */
void EffectKairikyRockSmoke::updateNormal( void )
{
  // エミッター
