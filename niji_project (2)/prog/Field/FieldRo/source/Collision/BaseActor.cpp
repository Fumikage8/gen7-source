...
void BaseActor::Clear(void)
{
  ...
  if( m_pCookingData )
  {
    GFL_SAFE_DELETE( m_pCookingData );
  }
}
...
*/
//-----------------------------------------------------------------------------
b32 BaseActor::CheckTrg(CollisionResult* pCollisionResult)
{
  for( u32 i=0 ; i<MAX_COLLISION_RESULT_CNT ; i++ )
  {
    if( m_resultOld[i].pTargetActor == pCollisionResult->pTargetActor )
    {
      return false;
    }
  }
                           ...
                           const f32                  e2)
{
  ...
  // 箱形状の作成
  ObbShape* obbShape    = GFL_NEW(m_pHeap) ObbShape();
  obbShape->m_pos       = pos;
  obbShape->m_u0        = u0;
  obbShape->m_u1        = u1;
  obbShape->m_u2        = u2;
...
*/
//-----------------------------------------------------------------------------
void BaseActor::CreateTriangle( const gfl2::math::Vector3 pos0,
                                const gfl2::math::Vector3 pos1,
                                const gfl2::math::Vector3 pos2)
{
  ...
  // メッシュ形状の作成
  TriangleShape* triangleShape =  GFL_NEW(m_pHeap) TriangleShape();
  triangleShape->m_pos0        = pos0;
  triangleShape->m_pos1        = pos1;
  triangleShape->m_pos2        = pos2;
  m_pShape                     = triangleShape;
  m_shape                      = SHAPE_TRIANGLE;
}

//----------------------------------------------------------------------------
/**
*  @brief  アクターにメッシュクッキングデータを追加
*/
//-----------------------------------------------------------------------------
void BaseActor::CreateCookingData(void)
{
#if USE_MESH_COOKING
  if( !m_pCookingData )
  {
    m_pCookingData = GFL_NEW( GetHeap() ) s32[MAX_COOKING_SIZE];
    for( u32 i=0 ; i<MAX_COOKING_SIZE ; i++ )
    {
      m_pCookingData[i] = -1;
    }
  }
