...
typedef struct {
    char m_Name[sNAMEMAX];
    u_int m_Hash;
} HashElem ;
static HashElem* s_hash_list = 0;

static u_int CalcHash( const char* str ) {
    u_int value = 0;
    while(*str) {
    ...
    {   // ハッシュの重複をチェック
        HashElem* he = s_hash_list;
        while(he->m_Name[0]){
            if(he->m_Hash == value){ // ハッシュが一致！
                if(strcmp(str,he->m_Name)){ // 名前が一致していない！　コリジョン発生！！
                    error(999,"Hash Collision!!");
                    return value;
                }
            }
            ++he;
        ...
        he->m_Hash = value;
        strcpy(he->m_Name,str);
    }
    return value;
}
static void WriteHash( FILE *fout, long* nameofs, const char* str ){
    u_int hsh = CalcHash(str);
    pc_writebin(fout,&hsh,sizeof(cell));
    *nameofs+=sizeof(cell);
}
...
SC_FUNC int assemble(FILE *fout,FILE *fin)
{
  ...
  cell mainaddr;
  char nullchar;
if( gUseHash || gOmitOffset ){
    if(!s_hash_list){
        s_hash_list = calloc(10000,sizeof(HashElem));
    }
}
...
if(gOmitOffset){
      func.nameofs=CalcHash(sym->name);
}else{
...
if(!gOmitOffset){
      ...
      if( gUseHash ){
          WriteHash(fout,&nameofs,sym->name);
      } else {
  ...
  if (pc_addlibtable) {
    ...
    for (constptr=libname_tab.next; constptr!=NULL; constptr=constptr->next) {
      if (constptr->value>0) {
        ...
        func.address=0;
if(gOmitOffset){
      func.nameofs=CalcHash(constptr->name);
}else{
        func.nameofs=nameofs;
}
        #if BYTE_ORDER==BIG_ENDIAN
...
if(!gOmitOffset){
        pc_resetbin(fout,nameofs);
if( gUseHash ){
        WriteHash(fout,&nameofs,constptr->name);
} else {
