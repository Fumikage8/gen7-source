...
void gfl::core::Math::AddXZRotateY( Vector* tag, const f32 yang, Vector& org )
{
	AddXZRotateY( &tag->GFL_VECTOR_X, &tag->GFL_VECTOR_Z, yang, org.GFL_VECTOR_X, org.GFL_VECTOR_Z );
}
void gfl::core::Math::AddXZRotateY_X( Vector* tag, const f32 yang, const f32 xorg )
{
	AddXZRotateY_X( &tag->GFL_VECTOR_X, &tag->GFL_VECTOR_Z, yang, xorg );
}
void gfl::core::Math::AddXZRotateY_Z( Vector* tag, const f32 yang, const f32 zorg )
{
	AddXZRotateY_Z( &tag->GFL_VECTOR_X, &tag->GFL_VECTOR_Z, yang, zorg );
}
bool gfl::core::Math::IsPositionFront( const Vector& org, const f32 org_yang, const Vector& tag )
{
	f32 s, c;
	gfl::core::Math::FSinCos( &s, &c, org_yang );
	return( 0.0f <= s * ( tag.GFL_VECTOR_X - org.GFL_VECTOR_X ) + c * ( tag.GFL_VECTOR_Z - org.GFL_VECTOR_Z ) );
}
bool gfl::core::Math::IsPositionFrontDegree90( const Vector& org, const f32 org_yang, const Vector& tag )
{
	f32 s, c;
	gfl::core::Math::FSinCos( &s, &c, org_yang );
	const f32 vx = tag.GFL_VECTOR_X - org.GFL_VECTOR_X;
	const f32 vz = tag.GFL_VECTOR_Z - org.GFL_VECTOR_Z;
	return gfl::core::Math::IsFrontDegree90( c * vx - s * vz, s * vx + c * vz );
}
...
f32 gfl::core::Math::OptimizeRadian( f32 rad )
{
	...
	if( ONE_PI-EPSILON < rad ){ // -PI < rad < PI
		rad -= TWO_PI;
	} else if( rad < -ONE_PI+EPSILON ){
		rad += TWO_PI;
	}
...
f32 gfl::core::Math::AddRadian( const f32 rad, const f32 add )
{
	...
	if( ONE_PI-EPSILON < ans ){
		return ans - TWO_PI;
	} else if( ans < -ONE_PI+EPSILON ){
		return ans + TWO_PI;
	} else {
		return ans;
	}
