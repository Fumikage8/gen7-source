...
namespace gfl { namespace maya { namespace plugins { namespace Shader {
  ...
  GF_ALIGNED(0x80) static const unsigned char  s_MultiTextureVertexShaderCode[] = {
#include "MultiTextureShader.vert.txt"
  };

  GF_ALIGNED(0x80) static const unsigned char  s_MultiTexturePixelShaderCode[] = {
#include "MultiTextureShader.frag.txt"
  };
  ...
  /// @brief	コンストラクタ
  //-----------------------------------------------------------------------------
  MultiTextureVertexShader::MultiTextureVertexShader()
  {
    //---- 頂点シェーダオブジェクトの作成 ----//
    m_hProgram = glCreateShader(GL_VERTEX_SHADER);

    Common::SecureData	data = Common::SecureData::DeCode( s_MultiTextureVertexShaderCode, sizeof(s_MultiTextureVertexShaderCode) );

    const char *vfile = (char*)s_MultiTextureVertexShaderCode;
  ...
  /// @brief	コンストラクタ
  //-----------------------------------------------------------------------------
  MultiTexturePixelShader::MultiTexturePixelShader()
  {
    //---- フラグメントシェーダオブジェクトの作成 ----//
    m_hProgram = glCreateShader(GL_FRAGMENT_SHADER);

    //ソースコードをシェーダオブジェクトに変換
    Common::SecureData	data = Common::SecureData::DeCode( s_MultiTexturePixelShaderCode, sizeof(s_MultiTexturePixelShaderCode) );

    const char* ffile = (char*)s_MultiTexturePixelShaderCode;
  ...
  /// @brief	
  //-----------------------------------------------------------------------------
  void MultiTexturePixelShader::vSetUpShader() const
  {
    //テクスチャユニット設定
