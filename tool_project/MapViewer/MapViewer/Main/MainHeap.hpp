...
static void HeapCreate(void * systemHeapAdrs,void * deviceHeapAdrs, size_t systemHeapSize, size_t devieHeapSize);
...
public:
  HeapObject()
  {
    ...
    m_addr_heap = malloc(m_SystemHeapTotalSize); 
    m_addr_dev  = malloc(m_DeviceHeapTotalSize);
    m_addr_win32_work = malloc( m_Win32WorkHeapSize );
    HeapCreate(m_addr_heap, m_addr_dev, m_SystemHeapTotalSize, m_DeviceHeapTotalSize);
  }
  ~HeapObject()
  {
    HeapFree();
  ...
  {
    *size = m_SystemHeapSize;
    return m_addr_heap;
  }
  void* GetDeviceHeapInfo(size_t* size)
  {
    *size = m_DeviceHeapSize;
    return m_addr_dev;
  }
  void* GetResidentHeapInfo(size_t* size)
  {
    *size = System::HeapSize::RESIDENT;
    return reinterpret_cast<u8*>(m_addr_heap)+m_SystemHeapSize;
  }
  void* GetResidentDeviceHeapInfo(size_t* size)
  {
    *size = System::HeapSize::RESIDENT_DEVICE;
    return reinterpret_cast<u8*>(m_addr_dev)+m_DeviceHeapSize;
  }
...
void HeapCreate(void * systemHeapAdrs,void * deviceHeapAdrs, size_t systemHeapSize, size_t devieHeapSize)
{
...
void HeapFree(void)
{
	gfl2::heap::Manager::DeleteHeap(gfl2::heap::Manager::GetHeapByHeapId( HEAPID_WIN32_WORK ));
