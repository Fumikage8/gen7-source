...
void nijiAllocator::Initialize( gfl2::heap::HeapBase* pHeap )
{
  ...
  for( int i=0; i<VRAM_ALLOCK_BLOCK_MAX; ++i ){ m_pResultResidentVramA[i] = gfl2::util::AreaMan::RESULT_FAILED; }
  for( int i=0; i<VRAM_ALLOCK_BLOCK_MAX; ++i ){ m_pResultResidentVramB[i] = gfl2::util::AreaMan::RESULT_FAILED; }
  ...
  for( int i=0; i<VRAM_ALLOCK_BLOCK_MAX; ++i ){ m_pResultApplicationVramA[i] = gfl2::util::AreaMan::RESULT_FAILED; }
  for( int i=0; i<VRAM_ALLOCK_BLOCK_MAX; ++i ){ m_pResultApplicationVramB[i] = gfl2::util::AreaMan::RESULT_FAILED; }
}
...
void nijiAllocator::DebugCheckApplicationVramClear()
{
  VramArea vramArea;
  GetApplication( &vramArea );
  GFL_ASSERT( (vramArea.vramA->GetContiniusFreeBlockNum() == GetBlockSize(vramArea.sizeA)) );
  GFL_ASSERT( (vramArea.vramB->GetContiniusFreeBlockNum() == GetBlockSize(vramArea.sizeB)) );
}
 ...
 */
nijiAllocator::nijiAllocator( gfl2::heap::HeapBase *devHeap, VramControlType controlType ) : 
...
void nijiAllocator::FreeVramArea( VramArea* p_area, void* const address, bool A )
{
  ...
  if( p_addrWk == NULL ){
    ...
    *p_addrWk = gfl2::util::AreaMan::RESULT_FAILED; // 初期化
  }
