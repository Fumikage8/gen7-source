...
#define VERSION_RED             4   ///< AGB ファイアーレッド
...
#define POKEMON_LANG_TAIWAN        9  ///<  言語コード：繁体字（台湾、香港）    TWN
#define POKEMON_LANG_CHINA         10 ///<  言語コード：簡体字（中国）      CHN
...
#else
#if 0 // @todo PM_VERSIONがないので
#define GET_VERSION()  PM_VERSION
#else
#define GET_VERSION()  VERSION_CTR_SAPPHIRE
#endif
#endif
...
//------------------------------------------------------------------
/**
 * @brief カロス地方かどうか  （ステータスやBOXなどでのカロス地方捕獲マークにて使用）
 * @param u8 version
 * 
 */
//------------------------------------------------------------------
#if 0   // @todo たぶん使わないので無効に 2015/02/10 by nakahiro
extern bool IsCalos( u8 version );
#endif
