...
const f32 LAYOUT_STRING_SCALE_X = 0.64f;    ///< 文字のXスケール値
const f32 LAYOUT_STRING_SCALE_Y = 0.68f;    ///< 文字のYスケール値
const f32 LAYOUT_NUMBER_SCALE_X = 0.56f;    ///< 数字のXスケール値（数字のみの場合はこちら）
const f32 LAYOUT_NUMBER_SCALE_Y = 0.68f;    ///< 数字のYスケール値（数字のみの場合はこちら）
...
int GetStringWidth( const wchar_t* str, const nw::font::Font *font, f32 fontWidth, int *retLine )
{
  ...
  while( *str != gfl2::str::EOM_CODE )
  {
    ...
    {
      // @todo タグを解析してより正確な文字幅を計算する必要がある
      // タグをスキップし、幅を計算
