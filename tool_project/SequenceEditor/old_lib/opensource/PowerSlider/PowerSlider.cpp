...
#include <cmath>
//#include <values.h> // for unix
// ‚Æ‚è‚ ‚¦‚¸ PowerSlider ‚É•K—v‚È’l‚ğİ’è
...
// VisualStudio ‚Í c99 ‚É€‹’‚µ‚Ä‚¢‚È‚¢‚Ì‚Å
#define round(val) floor(val+0.5)

void PowerSlider::setRange(double min, double max){
	int decimals = std::max<int>(int(round(std::abs(log(min-floor(min))/log(10.0)))),
													int(round(std::abs(log(max-floor(max))/log(10.0)))))+1;
	spinBox_->setDecimals(decimals);
	spinBox_->setRange(min, max);

	setSingleStep(std::min<int>(spinBox_->singleStep(),(max-min)/10.0));
}
...
void PowerSlider::setSingleStep(double step){
	...
	int decimals = int(round(std::abs(log(step-floor(step))/log(10.0))))+1;
	spinBox_->setDecimals(decimals);
