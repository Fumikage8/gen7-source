...
struct PAWNDefinition {
	...
	std::string key;
	std::string value;
	PAWNDefinition(int line_, const std::string &key_, const std::string &value_) :
...
public:
	LinePAWNState() : state(0), ifTaken(0), level(-1) {
	}
...
class PAWNStates {
	std::vector<LinePAWNState> vlls;
public:
	LinePAWNState ForLine(int line) {
		if ((line > 0) && (vlls.size() > static_cast<size_t>(line))) {
			return vlls[line];
		} else {
			return LinePAWNState();
		}
	}
	void Add(int line, LinePAWNState lls) {
		vlls.resize(line+1);
		vlls[line] = lls;
	}
};
...
struct OptionSetPAWN : public OptionSet<OptionsPAWN> {
	OptionSetPAWN() {
		...
		DefineProperty("fold.cpp.syntax.based", &OptionsPAWN::foldSyntaxBased,
		...
		DefineProperty("fold.cpp.comment.multiline", &OptionsPAWN::foldCommentMultiline,
			"Set this property to 0 to disable folding multi-line comments when fold.comment=1.");

		DefineProperty("fold.cpp.comment.explicit", &OptionsPAWN::foldCommentExplicit,
			"Set this property to 0 to disable folding explicit fold points when fold.comment=1.");

		DefineProperty("fold.cpp.explicit.start", &OptionsPAWN::foldExplicitStart,
			"The string to use for explicit fold start points, replacing the standard //{.");

		DefineProperty("fold.cpp.explicit.end", &OptionsPAWN::foldExplicitEnd,
			"The string to use for explicit fold end points, replacing the standard //}.");

		DefineProperty("fold.cpp.explicit.anywhere", &OptionsPAWN::foldExplicitAnywhere,
...
class LexerPAWN : public ILexer {
	...
	PAWNStates vlls;
	std::vector<PAWNDefinition> ppDefineHistory;
	WordList keywords;
	WordList keywords2;
	WordList keywords3;
	WordList keywords4;
...
public:
	...
	static ILexer *LexerFactoryPAWNInsensitive() {
		return new LexerPAWN(false);
	}
...
struct After {
	...
	bool operator() (PAWNDefinition &p) const {
		return p.line > line;
	}
};
...
void SCI_METHOD LexerPAWN::Lex(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
	...
	LinePAWNState preproc = vlls.ForLine(lineCurrent);
	...
	std::vector<PAWNDefinition>::iterator itInvalid = std::find_if(ppDefineHistory.begin(), ppDefineHistory.end(), After(lineCurrent-1));
	...
	for (std::vector<PAWNDefinition>::iterator itDef = ppDefineHistory.begin(); itDef != ppDefineHistory.end(); ++itDef) {
							...
							if (options.updatePreprocessor && !preproc.IsInactive()) {
								...
								if (restOfLine.find(")") == std::string::npos) {	// Don't handle macros with arguments
									...
									if (tokens.size() >= 1) {
										...
										ppDefineHistory.push_back(PAWNDefinition(lineCurrent, key, value));
										definitionsChanged = true;
									}
								}
							}
...
LexerModule lmPawn(SCLEX_PAWN, LexerPAWN::LexerFactoryPAWN, "cpp", cppWordLists);
//LexerModule lmPAWNNoCase(SCLEX_PAWNNOCASE, LexerPAWN::LexerFactoryPAWNInsensitive, "cppnocase", cppWordLists);
