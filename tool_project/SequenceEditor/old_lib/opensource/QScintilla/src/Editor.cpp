...
//==============================================================================
//==============================================================================
bool Editor::CheckWhiteSpace(const QString& str, int idx)
{
	int i;
	for( i = 0; i < idx; ++i ){
		if(str.at(i) != '\t'){
...
int Editor::GetStyle(const QString& str)
{
#if 0 // 
//	if(!str.indexOf( QRegExp( "//[^\n]*" ))){
	if(!str.indexOf( "//" )){
		return QsciLexerCPP::CommentLine;
	}

//	if(!str.indexOf( QRegExp( "\".*\"" ))){
//		return QsciLexerCPP::Comment;
//	}

	QStringList keywordPatterns;
	keywordPatterns << "\\bconst\\b" << "\\benum\\b" 
		<< "\\bforward\\b" << "\\boperator\\b" << "\\bstock\\b"
		<< "\\bnew\\b" << "\\bstatic\\b" << "\\bpublic\\b" << "\\bnative\\b";
	foreach( const QString &pattern, keywordPatterns )
	{
		if(!str.indexOf(QRegExp( pattern ))){
			return QsciLexerCPP::Keyword;
		}
	}

	keywordPatterns << "\\b#include\\b" << "\\b#define\\b" << "\\b#assert\\b"
		<< "\\b#else\\b" << "\\b#endif\\b" << "\\b#error\\b" << "\\b#assert\\b"
		<< "\\b#elseif\\b" << "\\b#tryinclude\\b" << "\\b#section\\b" << "\\b#undef\\b"
		<< "\\b#endinput\\b" << "\\b#file\\b" << "\\b#pragma\\b"
		;
	foreach( const QString &pattern, keywordPatterns )
	{
		if(!str.indexOf(QRegExp( pattern ))){
	...
	if(!str.indexOf( QRegExp( "\\b[A-Za-z0-9_]+(?=\\()" ))){
		return QsciLexerCPP::GlobalClass;
	}

	keywordPatterns.clear();
	keywordPatterns << "\\bif\\b" << "\\belse\\b" << "\\bcase\\b"
		<< "\\bassert\\b" << "\\bbreak\\b" << "\\bcontinue\\b"
		<< "\\bdefault\\b" << "\\bdo\\b" << "\\bexit\\b"
		<< "\\bfor\\b" << "\\bgoto\\b" << "\\breturn\\b"
		<< "\\bsleep\\b" << "\\bstate\\b" << "\\bswitch\\b"
		<< "\\bwhile\\b" << "\\bchar\\b" << "\\bdefined\\b"
		<< "\\bsizeof\\b" << "\\btagof\\b";
	foreach( const QString &pattern, keywordPatterns )
	{
		if(!str.indexOf(QRegExp( pattern ))){
			return QsciLexerCPP::Identifier;
		}
	}
	return QsciLexerCPP::Default;
#else // 
	int idx;
	idx = str.indexOf( "//" );
	if(!idx || CheckWhiteSpace(str,idx)){
		return QsciLexerCPP::CommentLine;
	}

	QStringList keywordPatterns;
	keywordPatterns << "const" << "enum" 
		<< "forward" << "operator" << "stock"
		<< "new" << "static" << "public" << "native";
	foreach( const QString &pattern, keywordPatterns )
	{
	...
	keywordPatterns.clear();
	keywordPatterns << "#include" << "#define" << "#assert" << "#if"
		<< "#else" << "#endif" << "#error" << "#assert"
		<< "#elseif" << "#tryinclude" << "#section" << "#undef"
		<< "#endinput" << "#file" << "#pragma"
		;
	foreach( const QString &pattern, keywordPatterns )
	{
		idx = str.indexOf( pattern );
		if(!idx || CheckWhiteSpace(str,idx)){
			return QsciLexerCPP::PreProcessor;
		}
	}

	keywordPatterns.clear();
	keywordPatterns << "if" << "case"
	...
	foreach( const QString &pattern, keywordPatterns )
	{
		idx = str.indexOf( pattern );
		if(!idx || CheckWhiteSpace(str,idx)){
			return QsciLexerCPP::DoubleQuotedString;
		}
	}
...
//			int styleMain = ll->styles[i];
			...
			// GameFreak
			int styleMain = GetStyle(ll->chars);
//			printf( "%s ->%d\n",ll->chars, styleMain);
