// This defines the interface to the QsciLexerPawn class.
//
...
#ifndef QSCILexerPawn_H
#define QSCILexerPawn_H
...
//! \brief The QsciLexerPawn class encapsulates the Scintilla C++
//! lexer.
class QSCINTILLA_EXPORT QsciLexerPawn : public QsciLexer
{
    Q_OBJECT
...
public:
    ...
    //! Construct a QsciLexerPawn with parent \a parent.  \a parent is typically
    //! the QsciScintilla instance.  \a caseInsensitiveKeywords is true if the
    //! lexer ignores the case of keywords.
    QsciLexerPawn(QObject *parent = 0, bool caseInsensitiveKeywords = false);

    //! Destroys the QsciLexerPawn instance.
    virtual ~QsciLexerPawn();
...
private:
    ...
    QsciLexerPawn(const QsciLexerPawn &);
    QsciLexerPawn &operator=(const QsciLexerPawn &);
};
