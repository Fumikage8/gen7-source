  ...
  namespace snd {
    ...
    class SoundSystem : public gfl::base::Singleton<SoundSystem> 
    {
      ...
      enum SoundArchiveType {
        SOUND_ARCHIVE_TYPE_ROM,     // ROM上のアーカイブを使用する
        SOUND_ARCHIVE_TYPE_MEMORY,  // メモリ上のアーカイブを使用する
#if GFL_HIO_ENABLE
        SOUND_ARCHIVE_TYPE_HIO,     // PC上のアーカイブを使用する
#endif
      }; 
      ...
      struct InitParam 
      {
        gfl::heap::HeapBase* pAppHeap;            // システムが使用するアプリケーションヒープ
        gfl::heap::HeapBase* pDevHeap;            // システムが使用するデバイスヒープ
        ...
        const char*          bcsarFilePath;       // サウンドアーカイブファイルのパス( SOUND_ARCHIVE_TYPE_HIO の場合は絶対パスを指定 )
